...
curl -fsSL http://mirrors.cloud.aliyuncs.com/linux/debian/gpg | sudo apt-key add -
add-apt-repository "deb [arch=amd64] http://mirrors.cloud.aliyuncs.com/linux/debian $(lsb_release -cs) stable"
